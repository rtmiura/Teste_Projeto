...
class Forca(object):
    ...
    def __init__(self,Lista_Jogadores):
        ...
        #time.sleep(1)
        dificuldade='D'#input('Em qual dificuldade você quer jogar? Fácil(F), Médio(M) ou Díficil(D) ')
        ...
        self.Lista_Jogadores=Lista_Jogadores.copy()
...
for i in range(len(Ranking_Jogadores_Aproveitamento)):
    print(f'{Colors.CYAN}N° {i+1:2.0f} - Jogador: {Ranking_Jogadores_Vitorias[i]:12s} - Aproveitamento {100*percentual_vitorias_jogador(dic_jogadores,Ranking_Jogadores_Vitorias[i]):4.2f} %{Colors.RESET}')
#time.sleep(5)
