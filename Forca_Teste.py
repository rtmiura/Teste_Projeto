...
class Jogador(object):
    ...
    def tenta_nova_letra(self):
        return input('\nDigite o teu palpite, uma letra da palavra secreta: ')
...
quant_de_jogadores = input('Você deseja jogar com 1 ou 2 jogadores: ')
dic_jogadores={}
    #Restart_jogo=False

#while not Restart_jogo:

if quant_de_jogadores == '1':
...
while Numero_tentativas>0:
        ...
        if not True in Vetor: #jogador Errou
            ...
            if (erros % 2) == 0:
                ...
                if Numero_tentativas<1: #checa se perdeu
                    print(f'Perdeu a palavra era {palavra_secreta.upper()}!')
                    dic_jogadores[lista_nome[0]].atualiza_derrota()
                else: #mostra o numero de tentaticas restantes
                    print(f'Você pode cometer mais {Numero_tentativas} erros')
            else:
                Numero_tentativas-=1 #remove tentativa
                ...
                if Numero_tentativas<1: #checa se perdeu
                    print(f'Perdeu a palavra era {palavra_secreta.upper()}!')
                    dic_jogadores[lista_nome[1]].atualiza_derrota()
                else: #mostra o numero de tentaticas restantes
                    print(f'Você pode cometer mais {Numero_tentativas} erros')
        ...
        if Palavra_Display==Palavra_formatada: #checa se ganhou
            if (erros % 2) == 0:
                print(f'Ganhou! A palavra secreta é {palavra_secreta.upper()}!')
                imprime_mensagem_vencedor()
                dic_jogadores[lista_nome[0]].atualiza_vitoria()
                dic_jogadores[lista_nome[0]].atualiza_n_tentativas_para_vencer(len(Letras_digitadas))
                break
            else:
                print(f'Ganhou! A palavra secreta é {palavra_secreta.upper()}!')
                imprime_mensagem_vencedor()
                dic_jogadores[lista_nome[1]].atualiza_vitoria()
                dic_jogadores[lista_nome[1]].atualiza_n_tentativas_para_vencer(len(Letras_digitadas))
                break
Restart_jogo = int(input('Quer continuar a jogar? Sim = 1, Não = 0 -'))!=1
